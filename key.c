...
//å‘é€å‘½ä»¤åŽ,æ£€æµ‹æŽ¥æ”¶åˆ°çš„åº”ç­?
//str:æœŸå¾…çš„åº”ç­”ç»“æž?
//è¿”å›žå€?:0,æ²¡æœ‰å¾—åˆ°æœŸå¾…çš„åº”ç­”ç»“æž?
//    å…¶ä»–,æœŸå¾…åº”ç­”ç»“æžœçš„ä½ç½?(strçš„ä½ç½?)
u8* check_cmd(u8 *str)
{
  char *strx=0;
  if(USART_RX_STA&0X8000)    //æŽ¥æ”¶åˆ°ä¸€æ¬¡æ•°æ®äº†
  { 
    USART_RX_BUF[USART_RX_STA&0X7FFF]=0;//æ·»åŠ ç»“æŸç¬?
    strx=strstr((const char*)USART_RX_BUF,(const char*)str);
  } 
  return (u8*)strx;
}
//å‘sim900aå‘é€å‘½ä»?
//cmd:å‘é€çš„å‘½ä»¤å­—ç¬¦ä¸?(ä¸éœ€è¦æ·»åŠ å›žè½¦äº†),å½“cmd<0XFFçš„æ—¶å€?,å‘é€æ•°å­?(æ¯”å¦‚å‘é€?0X1A),å¤§äºŽçš„æ—¶å€™å‘é€å­—ç¬¦ä¸².
//ack:æœŸå¾…çš„åº”ç­”ç»“æž?,å¦‚æžœä¸ºç©º,åˆ™è¡¨ç¤ºä¸éœ€è¦ç­‰å¾…åº”ç­?
//waittime:ç­‰å¾…æ—¶é—´(å•ä½:10ms)
//è¿”å›žå€?:0,å‘é€æˆåŠ?(å¾—åˆ°äº†æœŸå¾…çš„åº”ç­”ç»“æžœ)
//       1,å‘é€å¤±è´?
u8 send_cmd(u8 *cmd,u8 *ack,u16 waittime)
{
  ...
  if((u32)cmd<=0XFF)
  {
    while((USART1->SR&0X40)==0);//ç­‰å¾…ä¸Šä¸€æ¬¡æ•°æ®å‘é€å®Œæˆ?  
    USART1->DR=(u32)cmd;
  }else printf("%s\r\n",cmd);//å‘é€å‘½ä»?
  if(ack&&waittime)    //éœ€è¦ç­‰å¾…åº”ç­?
  {
    ...
    {
      delay_ms(10);
      if(USART_RX_STA&0X8000)//æŽ¥æ”¶åˆ°æœŸå¾…çš„åº”ç­”ç»“æžœ
      {
        if(check_cmd(ack))break;//å¾—åˆ°æœ‰æ•ˆæ•°æ® 
        USART_RX_STA=0;
      } 
    }
    if(waittime==0)res=1; 
...
u8 *Read_KeyValue(void)
{
  ...
  setrow0_1();delay_us(50);temp_byte[0] |= read_inputdata();//get input col value  
...
u8 send_key(void)
{
  ...
  else
  {
    ...
    if(byte[1]>APP_START && byte[1]<APP_END)
    {
      switch(byte[1])
      {
      case APP_G:  print_google(); return 0;
      case APP_B:  print_baidu(); return 0;
      case APP_N:  print_126(); return 0;
...
u8 app_fnc(u8 key)
{
  ...
  case key_8: return APP_8;  // change mode
  case key_9: return APP_9;  // clear
  case key_0: return APP_0;  // pair

  case key_g: return APP_G;  // www.google.com
  case key_b: return APP_B;  // www.baidu.com
  case key_n: return APP_N;  // www.126.com
  default: return 0;
  }
}
