...
u8* check_cmd(u8 *str)
{
	...
	{ 
		USART_RX_BUF[USART_RX_STA&0X7FFF]=0;//添加结束�?
		strx=strstr((const char*)USART_RX_BUF,(const char*)str);
	} 
	return (u8*)strx;
}
//向sim900a发送命�?
//cmd:发送的命令字符�?(不需要添加回车了),当cmd<0XFF的时�?,发送数�?(比如发�?0X1A),大于的时候发送字符串.
...
u8 send_cmd(u8 *cmd,u8 *ack,u16 waittime)
{
	...
	if((u32)cmd<=0XFF)
	{
		while((USART1->SR&0X40)==0);//等待上一次数据发送完�?  
		USART1->DR=(u32)cmd;
	}else printf("%s\r\n",cmd);//发送命�?
	if(ack&&waittime)		//需要等待应�?
	{
		while(--waittime)	//等待倒计�?
		{
			delay_ms(10);
...
u8 *Read_KeyValue(void)
{
	...
	setrow0_1();delay_us(50);temp_byte[0] |= read_inputdata();//get input col value	防抖�?
...
u8 *map_2_key(u8 *read_byte)
{
	...
	if(fn | app)
	{
		...
		if(fn)
		{
			for(i = 1; i < 7; i++)
			{
				//找到第一个其他按键键值，only response one fn function.返回给主发送程序�?
				if(byte[i])
				{
		...
		if(app)
		{
			for(i = 1; i < 7; i++)
			{
				//找到第一个其他按键键值，不让主发送程序响应，因为主程序只能发送一次按键，app需要支持复杂字串发�?
				if(byte[i])
				{
